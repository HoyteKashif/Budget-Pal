...
public class StaticBillDateHelper {
	 ...
	 */
	public static HashMap<Bill, HashMap<Month, List<LocalDate>>> getMapOfBillsToMonth() {
		HashMap<Bill, HashMap<Month, List<LocalDate>>> hshmapBillToPaycheckByMonth = new HashMap<>();
		// Iterate through the list of Monthly Bills
		for (Bill oBill : BillDB.getAllBillModels()) {
	 ...
	 * @param p_oHash
	 */
	public static void printBillDateHashMap(final HashMap<Bill, HashMap<Month, List<LocalDate>>> p_oHash) {
		for (Month oMonth : Month.values()) {
			System.out.println("\n**********  " + oMonth);
			for (Bill oBill : p_oHash.keySet()) {
